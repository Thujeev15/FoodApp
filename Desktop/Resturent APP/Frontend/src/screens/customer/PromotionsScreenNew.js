...
export default function PromotionsScreen({ navigation }) {
    ...
    const fetchPromotions = async () => {
        try {
            ...
            console.log('=== FETCHED PROMOTIONS ===');
            if (res.data.data && res.data.data.length > 0) {
                res.data.data.forEach(promo => {
                    console.log(`Promotion ${promo._id}: image = "${promo.image}"`);
                });
            }
    ...
    const handleImageError = (promoId, error) => {
        console.error(`Image failed for ${promoId}:`, error);
    ...
    const renderPromo = ({ item, index }) => {
        ...
        console.log(`Rendering promo ${item.title}: image="${item.image}", URI="${imageUri}", error=${hasError}`);

        return (
            <View style={styles.promoCard}>
                {item.image && !hasError ? (
                    <>
                        <Text style={styles.debugText}>{imageUri}</Text>
                        <Image 
                            source={{ uri: imageUri }}
                    ...
                    <View style={[styles.promoImage, styles.errorPlaceholder]}>
                        <Ionicons name="alert-circle" size={40} color="#999" />
                        <Text style={{ color: '#999' }}>Image failed</Text>
                    </View>
                ) : null}
                <LinearGradient
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={styles.emptyState}>
                        <Iconicons name="pricetag-outline" size={60} color={colors.textMuted} />
...
const styles = StyleSheet.create({
    ...
    debugText: { fontSize: 10, color: '#999', padding: 8, backgroundColor: '#f5f5f5' },
    promoImage: { width: screenWidth - 32, height: 200, resizeMode: 'cover' },
    errorPlaceholder: { backgroundColor: '#f0f0f0', justifyContent: 'center', alignItems: 'center' },
    promoGradient: { padding: 24, position: 'relative', minHeight: 180 },
    promoGradientOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: 24, paddingVertical: 16 },
    promoContent: {},
